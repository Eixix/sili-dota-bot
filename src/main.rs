...
use frankenstein::{AsyncApi, BotCommand, GetUpdatesParams, Message, ReplyParameters, SendAnimationParams, SendPollParams, SetMyCommandsParams, UpdateContent};
...
async fn connect_to_api(token: String) {
    ...
    let dodo_command = BotCommand::builder().command("dodo").description("Polls for dota availability").build();
    let set_my_commands_params = SetMyCommandsParams::builder().commands(vec![dodo_command]).build();
...
async fn process_message(message: Message, api: AsyncApi) {
    ...
    if let Some(ref message_content) = message.text {
        if message_content.contains("doubt") || message_content.contains("daut") {
            ...
            let send_animation_params = SendAnimationParams::builder().chat_id(CHAT_ID).animation(file_path).reply_parameters(reply_parameters).build();
...
async fn send_dodo_poll(api: AsyncApi, message: Option<Message>) {
    let result: Punlines = {
        let file_content = fs::read_to_string("resources/punlines.json").expect("Error reading punlines json");
    ...
    if let Some(message) = message {
        let reply_parameters = ReplyParameters::builder().message_id(message.message_id).build();
        let send_poll_params = SendPollParams::builder().reply_parameters(reply_parameters).chat_id(CHAT_ID).question("Dodo?").options(vec![yes_answer.clone(), no_answer.clone()]).build();
        if let Err(err) = api.send_poll(&send_poll_params).await {
            println!("Failed to send message: {err:?}");
        }
    } else {
        let send_poll_params = SendPollParams::builder().chat_id(CHAT_ID).question("Do".to_owned() + get_day_prefix()).options(vec![yes_answer.clone(), no_answer.clone()]).build();
        if let Err(err) = api.send_poll(&send_poll_params).await {
            println!("Failed to send message: {err:?}");
        }
    }
